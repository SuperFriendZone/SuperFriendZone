...
const CountStars = async () => {
  let StarsData = await fetch(
    `https://api.github.com/users/SuperFriendZone/starred?per_page=100&page=${page}`
  ).then((res) => res.json());
...
const WriteReadMe = async () => {
  ...
  let UserData = await fetch("https://api.github.com/users/SuperFriendZone").then(
    (res) => res.json()
  );

  //Creating the text what we gonna save on ReadMe file
  const text = `## Hi there 👋 <img align="right" src="https://avatars.githubusercontent.com/u/125986143?v=4" width="200" />
